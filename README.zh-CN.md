...
[English README](README.md) · [结构发现](docs/SCHEMA_DISCOVERY.md) · [抽象门槛与资料放置](docs/ABSTRACTION_READINESS.md) · [数据模型与数学说明](docs/DATA_MODEL.md) · [领域本体](docs/ONTOLOGIES.md) · [研究数据](research/) · [规模实验](research/SCALING_STUDY.md)
...
学习这个活动对人类来说常表现为新信息与既有结构之间建立可修正的对应。映射不是同一：它应保留差异、条件、来源、有效期和失效边界。系统因此把不同来源和视角分开保存，也把“知道什么”与“过去怎样成功找到它”的程序性记忆分开保存。
...
系统仍保留“原始性”：最先接纳的 `input` 层用 `is_initial` 标记为历史起点，但这个标记只表达时间和来源，不把它变成永久的语义根层，也不赋予检索优先权。默认只有当原始材料满足“节点数不少于 12”，并且同时满足“总字符数不少于 24000”或“短记录数不少于 50”时，LLM 才能开始结构发现或直接抽象。新的维度候选还必须在两次不同抽样中以同一命名空间 ID 重复出现，才能获批。`12 / 24000 / 50 / 2` 是可配置的 Alpha 工程基线，不是普遍科学定律；异质、高风险或长期数据应提高门槛。

对于外部资料和模型自己推导的知识，系统增加了独立、可审计的“资料放置计划”。同一来源和生命周期的真正续篇可以追加到已有层；不同作者、来源、版本、观点、权限或生命周期默认建立平行层；LLM 自己产生的抽象只能进入派生层，不能写回历史原始层；纯重复或引用只建映射；证据不足则等待人工复核。LLM 只提出方案，应用程序验证每个来源节点并要求批准。详见[抽象门槛与资料放置](docs/ABSTRACTION_READINESS.md)。
...
2. **检查抽象条件。** 样本规模和信息量未达到门槛时，不允许 LLM 从原始库抽象新层。
3. **结构勘探。** LLM 粗读轮换抽样的代表性样本，提出层、节点类型、属性和关系候选。
4. **本体差异与稳定性分析。** 系统识别已有类型、潜在同义项和新维度；新候选必须跨抽样重复出现。
5. **审批与资料放置。** 未经批准的候选不能进入本体；外部资料和模型派生资料另行形成可审计放置计划。
6. **按获批结构清洗和路由。** 将原始批次形成规范节点，分配到平行层，保存属性并指回来源。
7. **建立可替换向量索引。** 嵌入只用于候选检索；更换模型时可从规范节点重建。
8. **生成跨层候选。** 相似度寻找值得比较的元素，但不直接证明任何领域关系。
9. **按本体判断并验证关系。** 保存类型、方向、置信度、证据、属性与有效期；未知类型拒绝写入。
10. **提问时先检索小径层。** 可靠命中会给出起始层、关系过滤、深度、宽度和停止条件。
11. **小径命中则引导检索，未命中则自由探索。** 两者最终都产生可检查的证据图。
12. **执行有界跨层联想。** 用户设置最大关联深度；宽度、循环检测和信息增益共同限制扩展。
13. **形成结果。** 无生成模型时返回证据图；有模型时只能根据证据回答并引用真实节点。
14. **学习小径并审计。** 成功路线形成候选小径；本体发现、放置计划、查询和小径历史均可导出。
