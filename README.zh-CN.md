...
*一个与模型无关、可扩展到不同领域的多层级向量存储与检索系统，具有显式映射和一个平行的小径层——项目名称特意向约翰·洛克致意。*
...
这种开放性受到规则治理，而不是任意自由书写。关系定义会声明方向、逆关系、对称性、传递性、时间性、允许的端点类型、遍历权重和验证器。映射还可以携带开放属性与有效时间区间。模型可以选择一个已激活关系，也可以提出一个未知的、带命名空间的类型供人工审查；未知类型绝不会被静默写入。详见[可扩展领域本体](docs/ONTOLOGIES.md)。
...
只有达到可配置的语料规模门槛后，才允许模型辅助抽象（默认要求 `N ≥ 12` 且字符数 `C ≥ 24,000`，或者短记录数量 `N ≥ 50`）。一个新候选还必须在两轮采用不同样本的粗筛中重复出现，之后才可激活。这些是保守的 Alpha 默认值，不是普遍统计定律。第一个输入层会被标记为历史起源，但这个标记不会建立永久语义根层，也不会给予检索优先级。外部材料会得到一份可审计的放置建议：同一来源的延续内容可以追加；独立来源或视角默认创建平行层；机器抽象始终留在派生层。详见[抽象就绪条件与材料放置](docs/ABSTRACTION_READINESS.md)。
...
flowchart TB
    ...
    subgraph A4["D. 小径学习——程序记忆"]
        ...
        K -->|"自由或回退路径得到非空证据"| L["候选小径"]
        L -->|"同一内部有效路径被反复观察；Alpha 代理指标"| AS["已激活小径"]
        AS -.->|"未来的相似问题"| S
    end
```
...
小径选择会综合语义触发相似度、已观察到的可靠性和成熟度，并选择超过阈值的最高综合分。高置信匹配会在图扩展之前约束检索。局部匹配或不可信匹配使用自由探索；如果一条可信小径没有找到种子证据，同一次查询会立即回退到自由探索，而不是返回空答案。
...
用户设置最大联想深度。每次遍历都可以通过一个显式映射进入另一层。宽度限制每一步产生的新节点数；关系过滤限制哪些边合法；已访问节点追踪防止循环；较低的信息增益可以让探索在最大深度之前提前停止。因此，深度是一项研究预算，而不是让系统无限游荡的命令。
...
当自由探索或回退探索得到一个非空且内部一致的证据图后，应用程序会把路径总结为一条**候选**小径。相似的观察路径会强化这个候选；Alpha 参考实现在观察三次后将其提升为成熟小径。这只是程序有效性的代理指标，并不能证明答案正确。一条成熟路径会记录后续能够产生证据的成功、空路径失败和延迟；受引导检索失败时会回退到自由探索。明确的用户评价、验证器执行和自动退役仍属于后续工作。
...
最大深度、每步宽度、关系过滤、已访问节点循环控制和最低信息增益共同限制搜索。因此，联想深度是一项明确的计算预算。
...
建议的研究规定了对数尺度的数据点、冷启动与热启动运行、延迟百分位数、吞吐量、召回率、主张覆盖率、证据落地程度、小径错误路由率、信息深度、内存使用量和硬件披露。详见[完整规模扩展协议](research/SCALING_STUDY.md)和[可复现基准入口](benchmarks/)。
...
- 小径可能重复一条错误路径。候选成熟度、已观察可靠性和回退探索可以降低这种风险，但不能消除风险。验证器和失败条件在此 Alpha 版本中作为程序元数据保存，尚没有通用策略引擎去执行它们。
